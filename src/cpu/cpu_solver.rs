...
impl CpuKangarooSolver {
    ...
    pub fn new_full(pubkey: ProjectivePoint, start_bytes: [u8; 32], range_bits: u32, dp_bits: u32) -> Self {
    ...
    pub fn solve(&mut self, timeout: Duration) -> Option<Vec<u8>> {
        ...
        let jump_distances: Vec<Scalar> = (0..16)
            .map(|i| {
                ...
                let mask = if mean_exp >= 128 { u128::MAX } else { (1u128 << mean_exp) - 1 };
        ...
        let jump_points: Vec<ProjectivePoint> = jump_distances.iter().map(|s| ProjectivePoint::mul_by_generator(s)).collect();
